...
static void renderWithSplitInput(Shuffle &fx, const float *dry_left, const float *dry_right,
...
static void setup(Shuffle &fx)
{
  fx.setTempo(120.f);
  fx.setParameter(Shuffle::MIX, 1000);
  fx.setParameter(Shuffle::DECAY, 256);
  fx.setParameter(Shuffle::SYNC, Shuffle::SYNC_EVEN);
  fx.setParameter(Shuffle::HOLD, Shuffle::HOLD_GATE);
}
...
int main()
{
  ...
  // Shuffle freezes ~1 bar; at 120 BPM that is 2 s (96000 frames).
  ...
  std::printf("shuffle_preroll_rms=%.4f wet_rms=%.4f active=%d wet=%.3f %s\n",
  ...
  std::printf("shuffle_release wet=%.3f held=%d %s\n", fx.wetAmount(), fx.isPadHeld() ? 1 : 0,
              released ? "OK" : "FAIL");
  if (!released)
    ok = false;

  std::printf("shuffle_offline %s\n", ok ? "PASS" : "FAIL");
  return ok ? 0 : 1;
}

...
static void renderWithSplitInput(Retrig &fx, const float *dry_left, const float *dry_right,
...
static void setup(Retrig &fx)
{
  fx.setTempo(120.f);
  fx.setParameter(Retrig::MIX, 1000);
  fx.setParameter(Retrig::DECAY, 256);
  fx.setParameter(Retrig::SYNC, Retrig::SYNC_EVEN);
  fx.setParameter(Retrig::HOLD, Retrig::HOLD_GATE);
}
...
int main()
{
  ...
  std::printf("retrig_preroll_rms=%.4f wet_rms=%.4f active=%d wet=%.3f %s\n",
  ...
  std::printf("retrig_release wet=%.3f held=%d %s\n", fx.wetAmount(), fx.isPadHeld() ? 1 : 0,
              released ? "OK" : "FAIL");
  if (!released)
    ok = false;

  std::printf("retrig_offline %s\n", ok ? "PASS" : "FAIL");
  return ok ? 0 : 1;
}

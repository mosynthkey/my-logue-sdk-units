...
static void renderWithSplitInput(Reverse &fx, const float *dry_left, const float *dry_right,
...
static void setup(Reverse &fx)
{
  fx.setTempo(120.f);
  fx.setParameter(Reverse::MIX, 1000);
  fx.setParameter(Reverse::SYNC, Reverse::SYNC_EVEN);
  fx.setParameter(Reverse::HOLD, Reverse::HOLD_GATE);
}
...
int main()
{
  ...
  std::printf("reverse_preroll_rms=%.4f wet_rms=%.4f active=%d wet=%.3f %s\n",
  ...
  std::printf("reverse_release wet=%.3f held=%d %s\n", fx.wetAmount(), fx.isPadHeld() ? 1 : 0,
              released ? "OK" : "FAIL");
  if (!released)
    ok = false;

  std::printf("reverse_offline %s\n", ok ? "PASS" : "FAIL");
  return ok ? 0 : 1;
}

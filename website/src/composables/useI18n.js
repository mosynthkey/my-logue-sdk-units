...
export const japanesePluginDescriptions = {
  acidtouch: "新生KAOCIDです。タップで303風16ステップを生成し、XYはカットオフとレゾナンスだけを演奏します。ROOTは短音階の基準音（C1–C3）、DENは密度、ACIDはアクセントとスライド、MODEはソー／矩形とGate／Latch／Thru／Mutateです。Audio Inは置き換えます。既存のKaocidはそのまま残しています。",
  ...
  tapeosc: "tape motorの始動を再現するVarispeed oscillatorです。NTS-1 / microKORG2の帯域制限波形（saw / square / triangle / sine）をテープ速度で再生します。STARTは0-1023ms、CURVEは0-127（64=直線）。ノートオンでモーターが回り、離鍵後の音量はホストのアンプEGが決めます。WOWは0-100%です。",
  ...
  shuffle: "Glitch²風シャッフルです。約1小節のAUDIO INを取り込み、タッチでランダムな部分スライス（時々逆再生）を飛び回ります。Yは部分スライス長、Depthはwet。非タッチはバイパス。DECAYでリピート減衰します。",
  gater: "Glitch²風トランスゲートです。ライブ入力をテンポグリッドで切り刻みます（フリーズなし）。Yはゲート速度（下=遅い／上=速い）、Depthはwet。非タッチはバイパス。DECAYでエッジをなめます。",
  ...
  detunesawld: "デチューンソウのリードです。Xはキーとスケールに沿った1オクターブの音程、Yは隣接オクターブをクロスフェードする5オクターブ分の位置です。Depthはデチューン。PUMPは16ステップに同期してダックします。タッチでゲートです。",
  ...
  stepfilter: "テンポに同期し、周期的にランダムなCutoff値でフィルターをかけます。シンセパッドを使った長いコードのパターンに動きをつける時などにおすすめです。",
  ...
  steperror: "DataBendのメディア故障をステップごとに判定します。Xは発火確率、Yは破損の深さ（ホールド／クラッシュ／ドロップ／CDスキップ）。パッドを押しているあいだだけ効きます。",

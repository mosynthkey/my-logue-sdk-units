...
- **DetuneSawLd** (`detunesawld`) — synth（実験的）
  - 対応: nts-3_kaoss
  - デチューンソウ・リード。Xはキーとスケールに沿った1オクターブ、Yは隣接オクターブのクロスフェードで5オクターブ。PUMPは16分ステップに同期。
...
- **StepError** (`steperror`) — fx（実験的）
  - 対応: nts-3_kaoss
  - DataBend のメディア故障をテンポステップごとに判定。X=発火確率、Y=破損の深さ。タッチ中のみ。

- **StepFenv** (`stepfenv`) — fx（実験的）
  - 対応: nts-3_kaoss
  - テンポ同期のステップ・フィルタ・エンベロープ。各ステップでカットオフEG（アタック0、ディケイ可変、サスティン0、リリース0）。X=カットオフ、Y=エンベロープ深さ。タッチでグリッドリセット。

- **StepFilter** (`stepfilter`) — fx
  - 対応: nts-3_kaoss
  - テンポに同期し、周期的にランダムなCutoff値でフィルターをかけます。シンセパッドを使った長いコードのパターンに動きをつける時などにおすすめです。
...
- **TapeOsc** (`tapeosc`) — osc（実験的）
  - 対応: nts-1_mkii, microkorg2
  - テープモーター始動/停止つきバリスピード・オシレータ。プラットフォームの帯域制限波形をテープ速度で再生。START/STOPはms。NTS-1は鍵盤を離すと停止する。microKORG2はOSCにnote-offが無く、Releaseでは停止しない。

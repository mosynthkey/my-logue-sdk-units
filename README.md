...
- **NTS-3 idea pack (experimental)** — performance FX/OSC units that do not overlap stock NTS-3, Kaocid, airFM, TechnoRumble, or AirHorn. Remaining batch: BeatRepeat, RingExcit, WarpsMorph, EucGate, DataBend, ReesePhr, PercIter, GridsDrum, RevRoll, DredBass. Extra: EucRoll (Euclidean step roll), StepFenv (step filter envelope), StepDice, Trap808, UKGarage, BoomBap, Dembow, Footwork, BreakBeat, DnBass, Trance, Retrig, Reverse, Shuffle, Gater. Skipped as overlapping: Kick Rumble (#26 → TechnoRumble), Airhorn (#29 → AirHorn). Open the site with `?experimental` to preview them.
